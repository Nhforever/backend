...
const editConfig=(req, res) => {
    ...
    const sql = 'UPDATE pc_configs SET cpu=?,mother_board=?,house=?,ram=?,gpu=?,hdd=?,ssd=?,power_supply=?,price=?,cpu_cooler=?,in_stock=?,cat_id=?,sale=?,sale_=?,config_name=?,config_pic=COALESCE(NULLIF(?, ""), config_pic),description=?,active=? WHERE product_id=?;'
    db.query(sql,[cpu,mother_board,house,ram,gpu,hdd,ssd,power_supply,price,cpu_cooler,in_stock,cat_id,sale,sale_,config_name,config_pic,description,active,product_id], (err, result) => {
        if (err) {
            console.error('SQL Hiba:', err);
            return res.status(500).json({ error: 'Hiba az SQL-ben', details: err });
        }
