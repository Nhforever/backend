...
const editConfig=(req, res) => {
    const product_id=req.params.product_id;
    const { cpu,mother_board,house,ram,gpu,hdd,ssd,power_supply,price,cpu_cooler,in_stock,cat_id,sale,sale_,product_name,description,active } = req.body;
    ...
    db.query(sql,[cpu,mother_board,house,ram,gpu,hdd,ssd,power_supply,price,cpu_cooler,in_stock,cat_id,sale,sale_,product_name,product_pic,description,active,product_id], (err, result) => {
        if (err) {
            console.error('SQL Hiba:', err);
            return res.status(500).json({ error: 'Hiba az SQL-ben', details: err });
        }
